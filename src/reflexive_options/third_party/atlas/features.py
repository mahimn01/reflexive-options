...
# Inlined from trading_algo.quant_core.strategies.options.iv_rank to keep
# the vendored ATLAS feature pipeline self-contained. These three helpers
# are pure-numpy and have no trading-domain coupling beyond their formulas.
...
def iv_series_from_prices(
    ...
    Default fallback used when the caller does not supply an exogenous IV
    series. Uses a regime-dependent IV/RV ratio (high in calm regimes,
    compresses to ~1 during selloffs).
    """
...
def iv_rank(iv_series: np.ndarray, current_idx: int, lookback: int = 252) -> float:
    """IV Rank at a given index: (current - low) / (high - low) * 100."""
...
class ATLASFeatureComputer:
    def compute_features(
        ...
        lows: np.ndarray,
        volumes: np.ndarray,
        iv_series: Optional[np.ndarray] = None,
    ) -> np.ndarray:
        """Returns (T, 12) array of market features. Last 4 features (position state) added separately.

        If ``iv_series`` is provided it overrides the internal RV-based fallback;
        callers that have access to true implied-vol data (e.g. a simulated
        surface or market quotes) should pass it here.
        """
        ...
        # iv_est: estimated IV (caller-supplied or RV-based fallback)
        if iv_series is None:
            iv_est = iv_series_from_prices(closes, rv_window=30)
        else:
            iv_est = np.asarray(iv_series, dtype=np.float64)
            if iv_est.shape[0] != T:
                raise ValueError(
                    f"iv_series length {iv_est.shape[0]} != closes length {T}"
                )
        features[:, 5] = iv_est
