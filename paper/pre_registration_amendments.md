...
### A12 — Provenance annotation correction (OpenTimestamps anchor commit)

**Annotation-only; no hypothesis, method, or data change; still pre-data.** The amendment notice at the head of `pre_registration.md` (dated 2026-06-01) cites the original proof `pre_registration.md.v1.ots` as covering "commit `268c061`, 2026-05-02." The commit id in that parenthetical is a transcription error. `268c061` is the 2026-04-29 initial scaffolding commit, whose `pre_registration.md` hashes to `79957f08…` and carries **no** proof. The original proof `.v1.ots` in fact stamps hash `f968b22e…`, which is the `pre_registration.md` of **commit `63078f5`** (2026-05-02, the v0.3.0 lock where A1–A7 are closed). The post-amendment document (this file, with A8–A11) hashes to `ccae3ba2…` and is stamped by `pre_registration.md.ots` at **commit `764c2d3`** (2026-06-01). Both proofs predate any SPX data. The manuscript (`main.tex`) is corrected to cite `63078f5` as the anchor. `pre_registration.md` is deliberately left unedited to preserve its timestamped hash; this entry is the correction of record. Verify: `git show 63078f5:paper/pre_registration.md | shasum -a256` equals the file hash reported by `ots info paper/pre_registration.md.v1.ots` (`f968b22e…`).
