...
> **Amendment notice (2026-06-01, still pre-data).** Amendments **A8–A11** in `pre_registration_amendments.md` make structural corrections to this document *before any SPX data is loaded*. The primary hypothesis **H1 (§2) is superseded by a direct dealer-gamma (GEX) regression, H1′ (amendment A9)**, and the spectral **H4 (§3) is superseded by a critical-slowing-down early-warning test (amendment A8)** — the original spectral H4 targets a 5.3–11 yr Hopf period that is geometrically unresolvable in any window fitting the data. The Hawkes–SV section is repositioned and the tautological $n_{\mathrm{SV}}$ "verification" removed (A10). The §2 event-window dates, the §4 strike grid, and one reference DOI are corrected (A11). The original specifications are retained below for the historical record; they remain anchored in git history and in the original OpenTimestamps proof preserved as `pre_registration.md.v1.ots` (commit `268c061`, 2026-05-02), which proves the *original* pre-registration existed before data. This amended document is re-stamped and remains pre-data (WRDS acquisition: September 2026).
...
> **[Superseded by amendment A9 — direct dealer-gamma (GEX) regression H1′.]** The RL-agent surface tournament above routes the reflexivity claim through a trained Mamba+PPO+EWC policy, confounding the *model mechanism* with the *estimator* (a non-reflexive simulator wrapped in a sufficiently expressive agent could match equally well). The new primary test H1′ removes the agent and simulator from the inference path: it estimates signed dealer gamma directly from the SPX OI grid and tests the mechanism's sign-conditional forward-vol prediction on returns. The RL tournament is retained as a secondary/exploratory result. See `pre_registration_amendments.md` §A9.
...
- *Historical periods* (each defined as $t_{\text{event}} \pm 60$ trading days, $\approx 121$ trading days, $\approx 100$ rolling 21-day windows per event):
  ...
  |---|---|---|
  | Volmageddon | 2018-02-05 | 2017-11-07 → 2018-05-02 |
  | COVID crash | 2020-03-16 | 2019-12-17 → 2020-06-10 |
  | Yen carry unwind | 2024-08-05 | 2024-05-08 → 2024-10-29 |

  <!-- Dates corrected per amendment A11 to the authoritative `paper/event_windows.txt` (pandas_market_calendars NYSE ±60 trading days); the prior hand-edited dates differed by ~2 trading days. Plus a pre-specified quiet-regime control window 2017-05-01 → 2017-10-20 (A11). -->
...
| Strike grid (log-moneyness) | $k = \log(K/F) \in \{-0.20, -0.16, -0.12, -0.08, -0.04, 0, +0.04, +0.08, +0.12, +0.16, +0.20\}$ ($\Delta k = 0.04$, $K = 11$; corrected per amendment A11 to be consistent with the per-day dimension $M\times K = 7\times 11 = 77$ and per-window $21\times 77 = 1617$ below) |
...
**Pre-data amendments.** Amendments discovered through pre-validation work that does not touch the empirical data (e.g., detector validation on synthetic ground truth, metric resolution checks) are tracked separately in `paper/pre_registration_amendments.md` and are commit-hash anchored *before* the empirical data is loaded. The set of permissible amendments is closed at the commit that lands `pre_registration_amendments.md`. Any post-data deviation falls under the original deviations clause above.
...
The H4 detector is implemented in `src/reflexive_options/theory/spectral.py` and validated against synthetic ground truth (positive: limit-cycle Stuart-Landau, supercritical reflexive sim; negative: white noise, Heston) — see `runs/h4_validation/` for power curves at canonical settings. The validation surfaced four operational under-specifications in this section's locked spec; they are documented and re-locked in `paper/pre_registration_amendments.md` (A1–A4): adaptive Welch window resolution, dual signal ($|r_t|$ and realised-variance proxy $\widehat{v}_t$), random-permutation surrogate (replacing the under-specified "circular shuffle"), and a global-vs-local-dominance definition of `in_band`. Together these constitute the locked H4 protocol used at empirical evaluation time.

## 11. Connections to prior pre-registrations in finance/ML

Pre-registration is well-established in psychology and biomedical research (Camerer et al. 2018, *Nature Human Behaviour* 2, 637–644, replicating 21 social-science experiments with pre-registered protocols and finding markedly different effect-size distributions vs the original literature) and has a recognised institutional home in finance through the Pacific-Basin Finance Journal pre-registration pathway (Faff 2023, *Pacific-Basin Finance Journal* vol.\ 79, art.\ 101837, DOI 10.1016/j.pacfin.2022.101837 — DOI and attribution corrected per amendment A11; the prior `.101859` resolved to an unrelated paper) and through the AEA RCT Registry, which has hosted finance and market-microstructure RCTs since 2018 (https://www.aeaweb.org/journals/policies/rct-registry). Reproducibility-by-version-control is established in machine-learning workflows (Pineau et al. 2021 NeurIPS Reproducibility Checklist; PaperswithCode 2020 code-release recommendations at https://github.com/paperswithcode/releasing-research-code; Open RL Benchmark, Huang et al. arXiv:2402.03046, 2024). Recent systematic reviews of RL-in-finance (Bai et al. 2024, arXiv:2411.12746; Hoque et al. 2025, arXiv:2512.10913) survey the field's reproducibility challenges; we extend that survey by pre-committing the analysis pipeline at a known commit hash *before* empirical evaluation. <!-- Reworded 2026-04-22: dropped the prior attribution that these reviews "explicitly identify the lack of standardised robustness measurement and pre-committed analysis as a methodological deficit" — neither paper makes that specific claim. Citations retained as relevant survey context. --> To our knowledge, the present work is the first synthesis of [pre-registration + git commit-hash anchoring + block-bootstrap / BH-FDR statistical pre-spec + applied to RL-for-finance] in this subfield — a *combination* of established practices, not a discovery of any single one. The κ-sensitivity slope (H2) and the SW2-on-arbitrage-filtered-windows protocol (H1) are offered as reusable templates for future reflexive-finance work regardless of whether our specific H1 is supported. Full ingredient-by-ingredient comparison against PBFJ, AEA RCT, NeurIPS Checklist, Open RL Benchmark, Camerer et al., and Pérignon et al. (2024, *Review of Financial Studies* 37(11), 3558, on computational reproducibility in finance) is in `related_work.md` §4.
...
- §11: 2024 systematic review citation locked to Bai, Gao, Wan, Zhang & Song, *A Review of Reinforcement Learning in Financial Applications*, arXiv:2411.12746 (and the December 2025 follow-up at arXiv:2512.10913). Resolved.
