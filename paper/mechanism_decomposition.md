...
| Table 5 | 2.00 | skew | + | – | Same mechanism. The largest sign-disagreement in the table: Marketron Table 8 reports $+0.181$, our reflexive simulator gives $-0.257$ (the Heston baseline at the same config gives $-0.434$, so the reflexive feedback partially counteracts the baseline's negative-skew compounding). In an empirical setting where the underlying drift is reproduced (Phase 4 of the master TODO), this cell would likely flip positive — that prediction is now made before any real-data calibration touches it. |
